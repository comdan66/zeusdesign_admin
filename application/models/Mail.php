...
class Mail extends OaModel {
  ...
  public static function renderB2 ($text) { return ($text = trim ($text)) ? '<b style="font-weight: bold; color: #d22828;">' . $text . '</b>' : ''; }
  public static function renderContent ($text) { return ($text = trim ($text)) ? '<div style="display: block; font-size: 14px; line-height: 20px; border: 1px solid gainsboro; padding: 16px; margin-bottom: 12px; -moz-border-radius: 3px; -webkit-border-radius: 3px; border-radius: 3px; background-color: #fafafa;">' . $text . '</div>' : ''; }
  ...
  public static function renderLink ($text, $href = '') { if (is_array ($text)) { $href = isset ($text['href']) ? $text['href'] : (isset ($text[1]) ? $text[1] : ''); $text = isset ($text['text']) ? $text['text'] : (isset ($text[0]) ? $text[0] : ''); } return ($text = trim ($text)) ? ' <a' . ($href ? ' href="' . $href . '"' : '') . ' style="color: #4285f4; font-weight: normal; text-decoration: none; cursor: pointer; border-bottom: 1px solid #c6dafc;">' . $text . '</a> ' : ''; }

  public static function send ($tos, $title, $cb = null, $uri = '', $ccs = array ()) {
    if (!(is_string ($title) && ($title = trim ($title)))) return false;
    if (is_string ($cb)) { $t = $uri; $uri = $cb; $cb = $t; }
    ...
    if (!$tos = array_values (array_filter (array_map (function ($to) {
        if (is_object ($to) && ($to instanceof User)) return ((isset ($to->name) && isset ($to->email)) || (isset ($to->id) && ($to = User::find ('one', array ('select' => 'name,email', 'conditions' => array ('id = ?', $to)))))) && $to->email ? array ('name' => $to->name, 'email' => $to->email) : false;
        else if (is_numeric ($to) && ($to = User::find ('one', array ('select' => 'name,email', 'conditions' => array ('id = ?', $to)))) && $to->email) return array ('name' => $to->name, 'email' => $to->email);
    ...
    $ccs = array_values (array_filter (array_map (function ($cc) {
        if (is_object ($cc) && ($cc instanceof User)) return ((isset ($cc->name) && isset ($cc->email)) || (isset ($cc->id) && ($cc = User::find ('one', array ('select' => 'name,email', 'conditions' => array ('id = ?', $cc)))))) && $cc->email ? array ('name' => $cc->name, 'email' => $cc->email) : false;
        else if (is_numeric ($cc) && ($cc = User::find ('one', array ('select' => 'name,email', 'conditions' => array ('id = ?', $cc)))) && $cc->email) return array ('name' => $cc->name, 'email' => $cc->email);
