...
class PttGeter {
  ...
  public static function getListAndPrevNextUri ($uri, $tid = 0, $cookie = '') {
    ...
    for ($i = 0; $i < $divs->length; $i++) { 
      ...
      if ($count == '爆') {
        $count = 105;
      } else if (preg_match ('/^X.*/', $count)) {
        if (($t = str_replace ('X', '', $count)) == '')
          $t = 10;
        $count = ($t * -10 - 5);
      }
      ...
      array_push ($arr, array (
          ...
          'title' => $title,
          'uri' => $href,
          'date' => DateTime::createFromFormat ('m/d', $date)->format ('Y-m-d'),
          'author' => $author,
        ));
    }
    return array (
