<div class='panel'>
  ...
  <form class='form' method='post' action='<?php echo base_url ($uri_1, $obj->id);?>' enctype='multipart/form-data'>
    ...
    <div class='row n2'>
      <label>* 開啟方式</label>
      <div class='radios'>
        <label><input type='radio' name='target' value='<?php echo Banner::TARGET_SELF;?>' <?php echo (isset ($posts['target']) ? $posts['target'] : $obj->target) == Banner::TARGET_SELF ? ' checked' : '';?> /><span></span>本頁</label>
        <label><input type='radio' name='target' value='<?php echo Banner::TARGET_BLANK;?>' <?php echo (isset ($posts['target']) ? $posts['target'] : $obj->target) == Banner::TARGET_BLANK ? ' checked' : '';?>/><span></span>分頁</label>
      </div>
    </div>
