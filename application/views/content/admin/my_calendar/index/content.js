...
$(function () {
  ...
  function initMonth (y, m) {
    ...
    $.ajax ({
      url: 'https://dev.admin.zeusdesign.com.tw/admin/my_calendar/ajax',
      data: {},
      async: true, cache: false, dataType: 'json', type: 'POST'
    })
    .done (function (r) {
      r.map (function (s) {
        $month.find ('.days div[data-y="' + s.y + '"][data-m="' + s.m + '"][data-d="' + s.d + '"]').append (s.c.map (function (t) {
          return $('<div />').addClass ('type' + t.type).append (t.type == 2 ? $('<i />').addClass ('icon-shield') : $('<img />').attr ('src', t.img)).append ($('<span />').css ({'background-color': '#' + t.color}).text (t.text));
        }));
      });
    })
    .fail (function () {
