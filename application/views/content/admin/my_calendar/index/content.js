...
$(function () {
  var _panel = {
    ...
    close: function () {
      this.closeContent ().closeAdd ().clean ();
    ...
    closeContent: function () {
      this.$content.removeClass ('s');
      this.showList (true);
      return this;
    },
    showContent: function () {
      this.$content.text ('saddddddddddwwwwwwwwwww');
      ...
      this.$right.attr ('class', 'icon-checkmark').unbind ('click').click (function () { this.closeContent (); }.bind (this));
      this.$title.text ('修改行程');
    },
    closeAdd: function (save) {
      this.$add.removeClass ('s');
      this.showList (true);
      return this;
    },
    showAddForm: function () {
      ...
      var $r1 = $('<div />').addClass ('row').append (
                  $('<b />').addClass ('need').text ('行程標題')).append (
                  $('<input />').attr ('type', 'text').attr ('name', 'title').attr ('placeholder', '請輸入行程標題..').attr ('maxlength', '200').attr ('pattern', '.{1,200}').prop ('required', true).attr ('title', '輸入行程標題!'));

      var $r2 = $('<div />').addClass ('row').append (
                  $('<b />').text ('待辦事項')).append (
                  $('<div />').addClass ('mtckbs').append (
                    $('<span />').append (
                      $('<a />').click (function () {
      ...
      var $r3 = this.$datas.data ('tags').length ? $('<div />').addClass ('row').append (
                  ...
                  $('<div />').addClass ('rdos').append (this.$datas.data ('tags').map (function (t) {
                    return $('<label />').addClass ('radio').css ({'border-color': '#' + t.color}).append (
      ...
      var $r4 = $('<div />').addClass ('row').append (
                  ...
                  $('<div />').addClass ('ckbs').append (this.$datas.data ('users').map (function (t) {
                    return $('<label />').addClass ('checkbox').append (
                      $('<input />').attr ('type', 'checkbox').attr ('name', 'users').val (t.id)).append (
                      $('<span />')).append (t.name);
                  })));

      var $r5 = $('<div />').addClass ('row').append (
                  $('<b />').text ('備註')).append (
                  $('<textarea />').addClass ('pure').attr ('name', 'memo').attr ('placeholder', '請輸入備註..').attr ('title', '輸入行程備註!'));
    ...
    addSubmit: function () {
      var title = this.$add.find ('input[name="title"]').val ().trim ();
      if (!title.length) return this.$add.find ('.note').text ('未填寫標題！');

      var items = this.$add.find ('.mtckbs div[name="items"]').map (function () {
        return {
      ...
      var tag = this.$add.find ('.rdos input[type="radio"][name="tag"]').filter (function () { return $(this).prop ('checked') && $(this).val ().length; }).map (function () { return $(this).val (); }).toArray ();
      tag = tag.length ? tag[0] : 0;
      
      var users = this.$add.find ('.ckbs input[type="checkbox"][name="users"]').filter (function () { return $(this).prop ('checked') && $(this).val ().length; }).map (function () { return $(this).val (); }).toArray ();

      var memo = this.$add.find ('textarea[name="memo"]').val ().trim ();
    ...
    showAdd: function () {
      this.$add.empty ().append (this.showAddForm ());
    ...
    showList: function (back) {
      ...
      .done (function (r) {
        this.$list.removeClass ('loading').append (r.map (function (t) {
          return $('<span />').text (t.text).add ($('<div />').addClass (t.sort ? 'sort' : null).append (t.objs.map (this.initListItem.bind (this))))
        }.bind (this)));
      }.bind (this))
      .fail (function (r) {
        if ((t = window.fns.IsJsonString (r.responseText)) !== null) window.fns.tipText ({title: '設定錯誤！', message: t.message});
  ...
  $('.calendar').each (function () {
    ...
    // $that.find ('.days > div[data-y="2017"][data-m="6"][data-d="6"]').click ();
  });
