...
<div class='panel'>
  <form class='form-type1 loading' action='<?php echo base_url ($uri_1);?>' method='post'>
    ...
    <div class='row'>
      ...
      <input type='color' name='color' value='<?php echo isset ($posts['color']) ? '#' . $posts['color'] : '';?>' placeholder='請輸入<?php echo $title;?>顏色..' maxlength='200' pattern='.{1,200}' required title='輸入<?php echo $title;?>顏色!' autofocus />
    </div>
