...
<div class='panel'>
  <form class='form-type1 loading' action='<?php echo base_url ($uri_1);?>' method='post'>
    ...
    <div class='row min'>
      <b class='need'>是否入帳</b>
      <label class='switch'>
        <input type='checkbox' name='status'<?php echo (isset ($posts['status']) ? $posts['status'] : Outcome::STATUS_1) == Outcome::STATUS_2 ? ' checked' : '';?> value='<?php echo Outcome::STATUS_2;?>' />
        <span></span>
      </label>
    </div>
  ...
  <?php if ($users = User::all (array ('select' => 'id, name'))) {
    ...
    <div class='row'>
      ...
      <input type='date' name='date' value='<?php echo isset ($posts['date']) ? $posts['date'] : '';?>' placeholder='請輸入<?php echo $title;?>日期..' maxlength='200' pattern='.{1,200}' required title='輸入<?php echo $title;?>日期!' />
    </div>

    <div class='row'>
      <b class='need'><?php echo $title;?>金額</b>
      <input type='number' name='money' value='<?php echo isset ($posts['money']) ? $posts['money'] : '';?>' placeholder='請輸入<?php echo $title;?>金額..' maxlength='200' pattern='.{1,200}' required title='輸入<?php echo $title;?>金額!' />
    </div>
...
<?php foreach (Outcome::$typeNames as $key => $typeNames) { ?>
        <label class='radio'>
          <input type='radio' name='type' value='<?php echo $key;?>'<?php echo (isset ($posts['type']) ? $posts['type'] : Outcome::TYPE_1) == $key ? ' checked' : '';?>>
