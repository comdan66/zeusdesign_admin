...
class Wallets extends Api_controller {
  ...
  public function index () {
    ...
    foreach (Wallet::find ('all', array ('select' => 'id,title,money,address,cover,timed_at,DATE(timed_at) AS date', 'order' => 'timed_at DESC', 'conditions' => $conditions)) as $w)
      if ($d = array ('id' => $w->id, 'title' => $w->title, 'money' => $w->money, 'address' => $w->address, 'money_srt' => number_format ($w->money), 'cover' => $w->cover->url ('100x100c'), 'timed_at' => ($w->timed_at->format ('H') < 13 ? '上午' : '下午') . ' ' . $w->timed_at->format ('h點 i分')))
