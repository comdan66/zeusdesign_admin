...
class Users extends Admin_controller {
  ...
  public function __construct () {
    ...
    if (in_array ($this->uri->rsegments (2, 0), array ('update', 'show')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->user = User::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function show ($user_id = 0, $type = 'schedules', $offset = 0) {
    ...
    $configs = array ($this->uri_1, $user_id, 'show', $type, '%s');
    $conditions = conditions ($columns, $configs);
    $this->load->library ('pagination');
  ...
  public function update () {
    if (!$this->has_post ())
      return redirect_message (array ($this->uri_1, $this->user->id, 'edit'), array (
    ...
    if ($msg = $this->_validation ($posts))
      return $is_api ? $this->output_error_json ($msg) : redirect_message (array ($this->uri_1, $this->user->id, 'edit'), array (
          '_flash_danger' => $msg,
          'posts' => $posts
        ));
    ...
    $user = $this->user;
    $update = User::transaction (function () use ($user, $posts) { return $user->save (); });

    if (!$update)
      return $is_api ? $this->output_error_json ('更新失敗！') : redirect_message (array ($this->uri_1, $this->user->id, 'edit'), array (
          '_flash_danger' => '更新失敗！',
          'posts' => $posts
        ));
    ...
    if ($roles)
      foreach ($roles as $key => $bool)
        $bool ? (!UserRole::find ('one', array ('conditions' => array ('user_id = ? AND name = ?', $user->id, $key))) && UserRole::transaction (function () use ($user, $key) { return verifyCreateOrm (UserRole::create (array ('user_id' => $user->id, 'name' => $key))); })) : (($role = UserRole::find ('one', array ('conditions' => array ('user_id = ? AND name = ?', $user->id, $key)))) && UserRole::transaction (function () use ($role) { return $role->destroy (); }));

    return $is_api ? $this->output_json ($user->to_array ()) : redirect_message (array ($this->uri_1), array (
        '_flash_info' => '更新成功！'
      ));
  }
}
