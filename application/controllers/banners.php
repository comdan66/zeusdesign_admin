...
class Banners extends Admin_controller {
  ...
  private function _validation (&$posts) {
    ...
    if (isset ($posts['target']) && !(is_numeric ($posts['target'] = trim ($posts['target'])) && in_array ($posts['target'], array_keys (Banner::$targetNames)))) return '開啟方式錯誤！';
    if (isset ($posts['is_enabled']) && !(is_numeric ($posts['is_enabled'] = trim ($posts['is_enabled'])) && in_array ($posts['is_enabled'], array_keys (Banner::$enableNames)))) return '狀態錯誤！';
    return '';
  }
