...
class My_image_base_tags extends Admin_controller {
  private $uri_1 = null;
  private $obj = null;
  private $icon = null;
  ...
  public function __construct () {
    ...
    $this->uri_1 = 'admin/my-image-base-tags';
    $this->icon = 'icon-ta';

    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->obj = ImageBaseTag::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function index ($offset = 0) {
    ...
    $limit = 25;
    $total = ImageBaseTag::count (array ('conditions' => $conditions));
    $objs = ImageBaseTag::find ('all', array ('offset' => $offset < $total ? $offset : 0, 'limit' => $limit, 'include' => array ('image_bases'), 'order' => 'id DESC', 'conditions' => $conditions));
  ...
  public function create () {
    ...
    if (!ImageBaseTag::transaction (function () use (&$obj, $posts) { return verifyCreateOrm ($obj = ImageBaseTag::create (array_intersect_key ($posts, ImageBaseTag::table ()->columns))); }))
  ...
  public function update () {
    ...
    if (!ImageBaseTag::transaction (function () use ($obj, $posts) { return $obj->save (); }))
  ...
  public function destroy () {
    ...
    if (!ImageBaseTag::transaction (function () use ($obj) { return $obj->destroy (); }))
