...
class Works extends Admin_controller {
  ...
  public function index ($offset = 0) {
    ...
    $objs = Work::find ('all', array ('offset' => $offset, 'limit' => $limit, 'order' => 'id DESC', 'include' => array ('user'), 'conditions' => $conditions));
  ...
  private function _validation_create (&$posts, &$cover, &$images) {
    ...
    if (!is_upload_file_format ($cover, 2 * 1024 * 1024, array ('gif', 'jpeg', 'jpg', 'png'))) return '作品封面 格式錯誤！';
    ...
    $images = array_filter ($images, function ($image) { return is_upload_file_format ($image, 2 * 1024 * 1024, array ('gif', 'jpeg', 'jpg', 'png')); });
