...
class Articles extends Admin_controller {
  ...
  public function create () {
    ...
    if (($msg = $this->_validation_create ($posts, $cover)) || (!Article::transaction (function () use (&$obj, $posts, $cover) {
  ...
  public function status () {
    ...
    UserLog::logWrite (
      $this->icon,
      Article::$statusNames[$obj->status] . '一項' . $this->title,
      '將' . $this->title . '「' . $obj->mini_title () . '」調整為「' . Article::$statusNames[$obj->status] . '」',
      array ($backup, $obj->backup (true)));
