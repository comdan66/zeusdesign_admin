...
class Promos extends Admin_controller {
  ...
  public function index ($offset = 0) {
    $searches = array (
        'status'  => array ('el' => 'select', 'text' => '是否上架', 'sql' => 'status = ?', 'items' => array_map (function ($t) { return array ('text' => Promo::$statusNames[$t], 'value' => $t,);}, array_keys (Promo::$statusNames))),
        'target'  => array ('el' => 'select', 'text' => '鏈結開啟方式', 'sql' => 'target = ?', 'items' => array_map (function ($t) { return array ('text' => Promo::$targetNames[$t], 'value' => $t,);}, array_keys (Promo::$targetNames))),
    ...
    $objs = conditions ($searches, $configs, $offset, 'Promo', array ('order' => 'sort DESC, id DESC'));
  ...
  public function create () {
    ...
    $posts['sort'] = (($posts['sort'] = Promo::first (array ('select' => 'sort', 'order' => 'sort DESC'))) ? $posts['sort']->sort : 0) + 1;

    if (($msg = $this->_validation_create ($posts, $cover)) || (!Promo::transaction (function () use (&$obj, $posts, $cover) {
      if (!verifyCreateOrm ($obj = Promo::create (array_intersect_key ($posts, Promo::table ()->columns))))
        return false;
    ...
    UserLog::logWrite (
      $this->icon,
      '新增一項' . $this->title . '',
      '標題名稱為：「' . $obj->mini_title () . '」，內容是：「' . $obj->mini_content () . '」，點擊後使用 「' . Promo::$targetNames[$obj->target] . '」 的方式開啟 「' . $obj->mini_link () . '」',
      $obj->backup ());
  ...
  public function update () {
    ...
    if (!Promo::transaction (function () use ($obj, $posts, $cover) {
    ...
    UserLog::logWrite (
      $this->icon,
      '修改一項' . $this->title,
      '標題名稱為：「' . $obj->mini_title () . '」，內容是：「' . $obj->mini_content () . '」，使用「' . Promo::$targetNames[$obj->target] . '」的方式開啟「' . $obj->mini_link () . '」',
  ...
  public function status () {
    ...
    $validation = function (&$posts) {
      return !(isset ($posts['status']) && is_string ($posts['status']) && is_numeric ($posts['status'] = trim ($posts['status'])) && ($posts['status'] = $posts['status'] ? Promo::STATUS_2 : Promo::STATUS_1) && in_array ($posts['status'], array_keys (Promo::$statusNames))) ? '「設定上下架」發生錯誤！' : '';
    };
    ...
    if (!Promo::transaction (function () use ($obj, $posts) {
      return $obj->save ();
    })) return $this->output_error_json ('更新失敗！');

    UserLog::logWrite (
      $this->icon,
      Promo::$statusNames[$obj->status] . '一項' . $this->title,
      '將' . $this->title . '「' . $obj->mini_title () . '」調整為「' . Promo::$statusNames[$obj->status] . '」',
      array ($backup, $obj->backup (true)));

    return $this->output_json ($obj->status == Promo::STATUS_2);
  }
  public function sort ($offset = 0) {
    $searches = array ();
    $configs = array_merge (explode ('/', $this->uri_1), array ('sort', '%s'));
    $objs = conditions ($searches, $configs, $offset, 'Promo', array ('order' => 'sort DESC, id DESC'), null, 0);
  ...
  public function update_sort ($offset = 0) {
    ...
    $objs = array_combine (column_array ($objs = Promo::find ('all', array ('select' => 'id, sort, updated_at', 'conditions' => array ('id IN (?)', $posts['ids'] ? $posts['ids'] : array (0)))), 'id'), $objs);
    $c = count ($objs);
  ...
  private function _validation_create (&$posts, &$cover) {
    if (!(isset ($posts['status']) && is_string ($posts['status']) && is_numeric ($posts['status'] = trim ($posts['status'])) && in_array ($posts['status'], array_keys (Promo::$statusNames)))) $posts['status'] = Promo::STATUS_1;
    ...
    if (!(isset ($posts['target']) && is_string ($posts['target']) && is_numeric ($posts['target'] = trim ($posts['target'])) && in_array ($posts['target'], array_keys (Promo::$targetNames)))) return '「鏈結開啟方式」發生錯誤！';
